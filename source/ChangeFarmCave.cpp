...
/*
  ...
  * 编译命令：g++ @.cpp -o @.exe -mwindows
  * 源码编码：GB-2312 (受限于PowerShell可接受的中文编码)
  * 致看源码的程序员-我为什么使用C++程序：
  * - 脚本VBScript已过时，无法处理大文件；
  * - 脚本PowerShell虽然非常好用，但容易被报告为风险程序，需要包装；
  * - 构建C#程序、打包型Python程序需要用户安装.NET或Python，C#程序打包运行时可膨胀到130MB，再考虑到网络情况，对于该小功能来讲实属没必要；
  * - 作者水平有限，暂时想不到更易于分发、便于广大用户在Windows平台执行的程序，因此采用GNU套件构建C++程序，并内置PowerShell执行。
*/
#include <iostream>
#include <fstream>
#include <filesystem>
...
main
    ...
    ofs_ps1.close();
    string cmd = "powershell -ExecutionPolicy Bypass -File \"script_ChangeFarmCave_StardewValley_by_DreamyBlaze.ps1\"";
    STARTUPINFOA si{};
    si.cb = sizeof(si);
